...
// WarehousePayloadLine строка склада в payload документа.
...
// ParseWarehousePayload разбор JSON.
func ParseWarehousePayload(raw json.RawMessage) (*WarehousePayload, error) {
	if len(raw) == 0 {
		return nil, fmt.Errorf("пустой payload")
	}
	var p WarehousePayload
...
// ConsumeReservations потребляет резервы.
func (w *Warehouse) ConsumeReservations(ctx context.Context, tenant string, ids []uuid.UUID) error {
	for _, id := range ids {
