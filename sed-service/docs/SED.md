...
## Интеграция с sales-service (продажи/отгрузки)

Для согласования заказов клиента (SO) в тенанте создаются типы документов с **`warehouse_action: NONE`** (например код `SALES_ORDER_APPROVAL`; опционально `SHIPMENT_APPROVAL`).

После успешного **`POST /documents/:id/sign`** sed-service может вызвать **sales-service** (если заданы в конфиге `sales_callback_url` и `sales_callback_secret`):

- `POST {sales_callback_url}/api/v1/internal/sed-events`
