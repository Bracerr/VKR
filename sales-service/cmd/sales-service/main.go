// Package main точка входа sales-service.
//
...
func main() {
	...
	app := &usecases.App{Store: store, WH: wh, SED: sed, Trace: trace, TracePub: tracePub, Cfg: cfg}
	...
	if transport.UseKafka(eventMode) && len(brokers) > 0 {
		platformkafka.StartSedSignedConsumer(bgCtx, brokers, "sales-service", app.HandleSedDocumentSigned, log)
	}
