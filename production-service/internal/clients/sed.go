...
func (s *SED) GetDocument(ctx context.Context, bearer string, docID uuid.UUID) (*SedDocument, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, s.base+"/api/v1/documents/"+docID.String(), http.NoBody)
	if err != nil {
		return nil, err
	}
	req.Header.Set("Authorization", "Bearer "+bearer)
	...
	raw, _ := io.ReadAll(resp.Body)
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("sed get document: %d %s", resp.StatusCode, string(raw))
	}
	var out SedDocument
	if err := json.Unmarshal(raw, &out); err != nil {
		return nil, err
